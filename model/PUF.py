...
class PUF(AbstractPUF):
    """
    Arbiter PUF. can be initialise like this:
    -> PUF(number_of_stage) example: PUF(64)
    -> PUF(challenge_vector) example: PUF([0,0,1,0,1,1])
    -> PUF(serie_of_challenge) example: PUF([0,1,1,0],[1,1,1,1],[0,0,0,0]])
    """
    def __init__(self, *args):
        ...
        # Our PUF can be crate with the number_of_stage of state or with a challenge or with a serie of challenge
        if args == ():
            ...
            for arg in args:
                if type(arg) is list:
                    if type(arg[0]) is list:
                        print("** Creating the PUF. Initialisation with a series of challenge **")
                        self.series_of_challenge = arg
                    else:
                        print("** Creating the PUF. Initialisation with a challenge **")
                        self.challenge_vector = arg

                elif type(arg) in (int, float):
                    self.number_of_stage = arg
                    print(f"** Creating the PUF. Initialisation with {self.number_of_stage} stages **")

                else:
                    raise Exception("This type of attribute is not accept for the creation of PUF", arg)
        ...
        for attribute in (self.number_of_stage, self.challenge_vector, self.series_of_challenge):
            if attribute is not None:
                _set_attribute += 1
        if _set_attribute > 1:
            raise Exception("Only one parameter is need to create the PUF ")
        ...
        # create now the different stage of the PUF
        self.create_stage()
    ...
    def create_stage(self):
        """Create the different stage of our PUF"""
